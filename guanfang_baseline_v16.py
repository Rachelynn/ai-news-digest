...
def generate_train_data(batch_dir, output_dir, merge=False, big_threshold=10000):
    ...
    if merge and success_count > 0:
        merge_path = os.path.join(output_dir, 'all_100.xlsx')
        ...
        if xlsx_files:
            print(f"\n[*] 合并 {len(xlsx_files)} 个文件 → all_100.xlsx...")
            dfs = []
            for f in xlsx_files:
                try:
            ...
            if dfs:
                ...
                df_all.to_excel(merge_path, index=False, engine='openpyxl')
                print(f"✅ 合并完成: {len(df_all)} 行 x {len(df_all.columns)} 列")
                return merge_path
    return None
...
def task2_capital_recognition(df_feat):
    ...
    for gid, gdf in df.groupby('behavior_group'):
        ...
        for i in gdf_norm.index:
            ...
            # 强制转成标量再存储
            scores.append((float(sy), float(sq)))
