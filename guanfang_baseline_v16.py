...
# ============================================================
# PART 0: LSTM/Transformer 时序建模（提分路径五增强版）
# ============================================================

# 时序模型使用的原始快照特征列（数值型，无需聚合）
TS_RAW_FEATURES = [
    'price', 'volume', 'amount',
    'totalbidvolume', 'totalaskvolume',
    'bidaskdifference', 'bidaskrate',
    'weightedbidprice', 'weightedaskprice',
    'bigordervolume',
    'tick_volume', 'tick_amount',
]

TS_SEQ_LEN = 256  # 固定序列长度（截断/填充）
TS_HIDDEN_DIM = 64
TS_NUM_LAYERS = 2
TS_DROPOUT = 0.2
TS_EPOCHS = 30
...
class StockSequenceDatasetMultiDay(Dataset):
    """多只股票多天的时序数据集（滑动窗口版）"""
    def __init__(self, df_raw, pseudo_labels, seq_len=TS_SEQ_LEN, window_days=5,
                 feature_cols=TS_RAW_FEATURES):
        self.seq_len = seq_len
        self.window_days = window_days
        self.feature_cols = [c for c in feature_cols if c in df_raw.columns]
        self.samples = []

        # 按股票分组，每只股票内部按日期排序
        label_map = {}
        for _, row in pseudo_labels.iterrows():
            key = (row['stock_code'], row['transaction_date'])
            label_map[key] = {
                'capital': {'游资': 0, '量化': 1, '散户': 2}.get(row['capital_type'], 1),
                'intent': {'买入': 0, '中性': 1, '卖出': 2}.get(row['capital_intention'], 1)
            }

        for sc, g_stock in df_raw.groupby('stock_code'):
            g_stock = g_stock.sort_values(['transaction_date', 'datetime'])
            dates = g_stock['transaction_date'].unique()
            if len(dates) < window_days:
                continue  # 该股票天数不足，跳过

            # 滑动窗口：取连续 window_days 天
            for i in range(window_days - 1, len(dates)):
                target_date = dates[i]
                window_dates = dates[i - window_days + 1 : i + 1]
                g_window = g_stock[g_stock['transaction_date'].isin(window_dates)]
                if len(g_window) == 0:
                    continue
                ...
                feat = g_window[self.feature_cols].values.astype(np.float32)
                # 截断或填充到固定长度
                if len(feat) >= seq_len:
                    feat = feat[-seq_len:]
                else:
                    pad = np.zeros((seq_len - len(feat), len(self.feature_cols)), dtype=np.float32)
                    feat = np.vstack([pad, feat])
                feat = self._normalize(feat)
                self.samples.append((feat, label_map[key]['capital'], label_map[key]['intent'], key))
    ...
    def __getitem__(self, idx):
        feat, capital, intent, key = self.samples[idx]
        return (torch.FloatTensor(feat),
                torch.LongTensor([capital])[0],
                torch.LongTensor([intent])[0],
                key)


class StockSequenceDataset(Dataset):
    """股票快照序列数据集（单天版，多只股票多天训练）"""
    def __init__(self, df_raw, pseudo_labels, seq_len=TS_SEQ_LEN, feature_cols=TS_RAW_FEATURES):
        self.seq_len = seq_len
        self.feature_cols = [c for c in feature_cols if c in df_raw.columns]
        self.samples = []
        ...
        for (sc, td), g in grouped:
            key = (sc, td)
            if key not in label_map:
                continue
            g = g.sort_values('datetime')
            feat = g[self.feature_cols].values.astype(np.float32)
            # 截断或填充到固定长度
            if len(feat) >= seq_len:
                feat = feat[-seq_len:]  # 取最后 seq_len 个时间点
            else:
                pad = np.zeros((seq_len - len(feat), len(self.feature_cols)), dtype=np.float32)
                feat = np.vstack([pad, feat])  # 前面填充 0
            # 归一化
            feat = self._normalize(feat)
            self.samples.append((feat, label_map[key]['capital'], label_map[key]['intent'], key))
...
class TemporalLSTM(nn.Module):
    """LSTM 时序分类器：输入快照序列，输出参与者类型 + 意图"""
    def __init__(self, input_dim, hidden_dim=TS_HIDDEN_DIM, num_layers=TS_NUM_LAYERS,
                 num_capital=3, num_intent=3, dropout=TS_DROPOUT):
        super().__init__()
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers,
                           batch_first=True, dropout=dropout if num_layers > 1 else 0)
        self.capital_fc = nn.Linear(hidden_dim, num_capital)
        self.intent_fc = nn.Linear(hidden_dim, num_intent)
    ...
    def forward(self, x):
        ...
        capital_logits = self.capital_fc(last_hidden)
        intent_logits = self.intent_fc(last_hidden)
        return capital_logits, intent_logits


class TemporalTransformer(nn.Module):
    """轻量 Transformer 时序分类器"""
    def __init__(self, input_dim, hidden_dim=TS_HIDDEN_DIM, num_layers=TS_NUM_LAYERS,
                 num_capital=3, num_intent=3, dropout=TS_DROPOUT, nhead=4):
        super().__init__()
    ...
    def forward(self, x):
        ...
        capital_logits = self.capital_fc(pooled)
        intent_logits = self.intent_fc(pooled)
        return capital_logits, intent_logits


def train_temporal_model(df_raw, pseudo_labels, model_type='lstm', multi_day=False, window_days=5, epochs=TS_EPOCHS):
    """训练时序模型，支持单天版和多天滑动窗口版
    
    Args:
        multi_day: True 使用跨天滑动窗口（一只股票连续多天），False 使用单天
        window_days: multi_day=True 时的滑动窗口天数
    """
    if not TORCH_AVAILABLE:
        print("[!] PyTorch 未安装，跳过时序模型训练")
        return None, None

    if multi_day:
        dataset = StockSequenceDatasetMultiDay(df_raw, pseudo_labels, window_days=window_days)
        print(f"\n>>> 使用跨天滑动窗口模式 | window={window_days}天")
    else:
        dataset = StockSequenceDataset(df_raw, pseudo_labels)
        print(f"\n>>> 使用单天模式")

    if len(dataset) < 10:
        print(f"[!] 时序训练样本不足 ({len(dataset)} < 10)，跳过")
        return None, None

    dataloader = DataLoader(dataset, batch_size=TS_BATCH_SIZE, shuffle=True)
    input_dim = len(dataset.feature_cols)

    if model_type == 'lstm':
        model = TemporalLSTM(input_dim)
    else:
        model = TemporalTransformer(input_dim)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=TS_LR)
    criterion = nn.CrossEntropyLoss()

    print(f">>> 训练 {model_type.upper()} 时序模型 | 样本={len(dataset)} | 设备={device}")

    model.train()
    for epoch in range(epochs):
        total_loss = 0
        for feat, capital_y, intent_y, _ in dataloader:
            feat = feat.to(device)
            capital_y = capital_y.to(device)
            intent_y = intent_y.to(device)

            optimizer.zero_grad()
            capital_logits, intent_logits = model(feat)
            loss = criterion(capital_logits, capital_y) + criterion(intent_logits, intent_y)
            loss.backward()
            optimizer.step()
    ...
    model.eval()
    predictions = {}
    infer_loader = DataLoader(dataset, batch_size=TS_BATCH_SIZE, shuffle=False)
    with torch.no_grad():
        for feat, _, _, keys in infer_loader:
            feat = feat.to(device)
            capital_logits, intent_logits = model(feat)
            capital_probs = torch.softmax(capital_logits, dim=1).cpu().numpy()
            intent_probs = torch.softmax(intent_logits, dim=1).cpu().numpy()
            capital_preds = capital_logits.argmax(dim=1).cpu().numpy()
            intent_preds = intent_logits.argmax(dim=1).cpu().numpy()

            for i, key in enumerate(keys):
                predictions[key] = {
                    'capital_pred': int(capital_preds[i]),
                    'intent_pred': int(intent_preds[i]),
                    'capital_prob': capital_probs[i].tolist(),
                    'intent_prob': intent_probs[i].tolist()
                }

    print(f"  时序模型推理完成 | 预测 {len(predictions)} 条记录")
    return model, predictions




def merge_three_model_predictions(df_res, preds_a, preds_b, alpha_static=0.4, alpha_temporal_a=0.35, alpha_temporal_b=0.25):
    """三模型融合：静态 + 单天时序(模式A) + 跨天时序(模式B)"""
    capital_names = {0: '游资', 1: '量化', 2: '散户'}
    intent_names = {0: '买入', 1: '中性', 2: '卖出'}
    
    # 归一化权重
    total = alpha_static + alpha_temporal_a + alpha_temporal_b
    w_s = alpha_static / total
    w_a = alpha_temporal_a / total
    w_b = alpha_temporal_b / total
    ...
    for _, row in df_res.iterrows():
        ...
        # 静态模型概率（one-hot）
        static_capital = row['capital_type']
        static_intent = row['capital_intention']
        static_conf = row.get('capital_confidence', 0.5)
        static_cap_prob = [0.0, 0.0, 0.0]
        static_int_prob = [0.0, 0.0, 0.0]
        cap_idx = {'游资': 0, '量化': 1, '散户': 2}.get(static_capital, 1)
        int_idx = {'买入': 0, '中性': 1, '卖出': 2}.get(static_intent, 1)
        static_cap_prob[cap_idx] = static_conf
        static_cap_prob[(cap_idx + 1) % 3] = (1 - static_conf) / 2
        static_cap_prob[(cap_idx + 2) % 3] = (1 - static_conf) / 2
        static_int_prob[int_idx] = static_conf
        static_int_prob[(int_idx + 1) % 3] = (1 - static_conf) / 2
        static_int_prob[(int_idx + 2) % 3] = (1 - static_conf) / 2

        # 模式A：单天时序
        a_cap_prob = static_cap_prob.copy()
        a_int_prob = static_int_prob.copy()
        if preds_a and key in preds_a:
            a_cap_prob = preds_a[key]['capital_prob']
            a_int_prob = preds_a[key]['intent_prob']
        
        # 模式B：跨天时序
        b_cap_prob = static_cap_prob.copy()
        b_int_prob = static_int_prob.copy()
        if preds_b and key in preds_b:
            b_cap_prob = preds_b[key]['capital_prob']
            b_int_prob = preds_b[key]['intent_prob']
        
        # 加权融合概率
        final_cap_prob = [w_s * static_cap_prob[i] + w_a * a_cap_prob[i] + w_b * b_cap_prob[i] for i in range(3)]
        final_int_prob = [w_s * static_int_prob[i] + w_a * a_int_prob[i] + w_b * b_int_prob[i] for i in range(3)]
        
        final_capital = capital_names[final_cap_prob.index(max(final_cap_prob))]
        final_intent = intent_names[final_int_prob.index(max(final_int_prob))]
        final_conf = max(final_cap_prob)
        
        row = row.copy()
        row['capital_type'] = final_capital
        row['capital_intention'] = final_intent
        row['capital_confidence'] = final_conf
        merged.append(row)

    return pd.DataFrame(merged)


def merge_temporal_predictions(df_res, temporal_preds, alpha=0.5):
    """融合静态模型和时序模型的预测结果（兼容旧版）"""
    if temporal_preds is None or len(temporal_preds) == 0:
        print("[!] 无时序预测结果，使用静态模型")
        return df_res
    ...
    for _, row in df_res.iterrows():
        ...
        if key in temporal_preds:
            tp = temporal_preds[key]
            # 时序模型预测
            temp_capital = capital_names[tp['capital_pred']]
            temp_intent = intent_names[tp['intent_pred']]
            temp_conf = max(tp['capital_prob'])

            # 加权融合（alpha: 时序权重）
            final_conf = alpha * temp_conf + (1 - alpha) * static_conf

            # 如果时序模型置信度显著高于静态，采用时序结果
            if temp_conf > static_conf + 0.15:
                final_capital = temp_capital
                final_intent = temp_intent
            else:
                final_capital = static_capital
                final_intent = static_intent
        else:
            final_capital = static_capital
            final_intent = static_intent
...
KEY_COLS_BASELINE = [
    ...
    'auctionvolume',
    # ========== 时序特征（提分路径五）==========
    'ts_price_trend', 'ts_price_volatility', 'ts_max_drawdown', 'ts_max_rally',
    'ts_vol_cv', 'ts_vol_autocorr1', 'ts_vol_autocorr5',
    'ts_buy_morning_pct', 'ts_buy_noon_pct', 'ts_buy_afternoon_pct',
    'ts_buy_trend', 'ts_imbalance_volatility', 'ts_big_order_burst',
    'ts_price_skew', 'ts_price_kurt', 'ts_return_volatility',
    'ts_morning_rally', 'ts_afternoon_rally', 'ts_vwap_trend',
    'ts_trade_pace_speedup', 'ts_cancel_acceleration', 'ts_fast_cancel_surge',
    'ts_vol_shift',
]
...
def extract_temporal_features(g, f):
    """
    提取日内时序特征（提分路径五：时间序列建模）。
    基于原始快照序列 g 计算动态行为特征，捕捉"先吸筹后拉升"等时序模式。
    所有特征以单标量形式加入特征字典，供下游聚类/分类使用。
    """
    n = len(g)
    if n < 3:
        # 数据过少，时序特征置零
        for k in ['ts_price_trend', 'ts_price_volatility', 'ts_max_drawdown', 'ts_max_rally',
                  'ts_vol_cv', 'ts_vol_autocorr1', 'ts_vol_autocorr5',
                  'ts_buy_morning_pct', 'ts_buy_noon_pct', 'ts_buy_afternoon_pct',
                  'ts_buy_trend', 'ts_imbalance_volatility', 'ts_big_order_burst',
                  'ts_price_skew', 'ts_price_kurt', 'ts_return_volatility',
                  'ts_morning_rally', 'ts_afternoon_rally', 'ts_vwap_trend',
                  'ts_order_urgency', 'ts_trade_pace_speedup', 'ts_cancel_acceleration']:
            f[k] = 0.0
        return f
    ...
    if np.std(prices) > 1e-6:
        ...
        f['ts_price_trend'] = slope / (np.mean(prices) + 1e-8)
    else:
        f['ts_price_trend'] = 0.0

    # 2. 价格波动率（日内收益率标准差）
    f['ts_price_volatility'] = np.std(returns) if len(returns) > 0 else 0.0

    # 3. 最大回撤 & 最大反弹
    cummax = np.maximum.accumulate(prices)
    drawdowns = (cummax - prices) / (cummax + 1e-8)
    f['ts_max_drawdown'] = np.max(drawdowns) if len(drawdowns) > 0 else 0.0
    cummin = np.minimum.accumulate(prices)
    rallies = (prices - cummin) / (cummin + 1e-8)
    f['ts_max_rally'] = np.max(rallies) if len(rallies) > 0 else 0.0

    # 4. 价格偏度/峰度（捕捉极端走势）
    if len(prices) > 3 and np.std(prices) > 1e-6:
        z = (prices - np.mean(prices)) / (np.std(prices) + 1e-8)
        f['ts_price_skew'] = np.mean(z ** 3)
        f['ts_price_kurt'] = np.mean(z ** 4) - 3.0
    else:
        f['ts_price_skew'] = 0.0
        f['ts_price_kurt'] = 0.0

    # ---- 成交量序列特征 ----
    tick_vols = g['tick_volume'].values.astype(float)
    if 'tick_amount' in g.columns:
        tick_amts = g['tick_amount'].values.astype(float)
    else:
        tick_amts = tick_vols * prices

    f['ts_vol_cv'] = np.std(tick_vols) / (np.mean(tick_vols) + 1e-8) if np.mean(tick_vols) > 0 else 0.0

    # 成交量自相关（lag-1, lag-5）
    if len(tick_vols) > 5 and np.std(tick_vols) > 1e-6:
        v_norm = (tick_vols - np.mean(tick_vols)) / (np.std(tick_vols) + 1e-8)
        f['ts_vol_autocorr1'] = np.corrcoef(v_norm[:-1], v_norm[1:])[0, 1] if len(v_norm) > 1 else 0.0
        f['ts_vol_autocorr5'] = np.corrcoef(v_norm[:-5], v_norm[5:])[0, 1] if len(v_norm) > 5 else 0.0
    else:
        f['ts_vol_autocorr1'] = 0.0
        f['ts_vol_autocorr5'] = 0.0

    # ---- 时间窗口行为特征（早盘/午盘/尾盘） ----
    # 使用 g 中的 datetime 或 hour/minute 字段
    if 'hour' in g.columns and 'minute' in g.columns:
        hours = g['hour'].values
        minutes = g['minute'].values
    elif 'datetime' in g.columns:
        dt_local = g['datetime'] + pd.Timedelta(hours=8)
        hours = dt_local.dt.hour.values
        minutes = dt_local.dt.minute.values
    else:
        hours = np.zeros(n)
        minutes = np.zeros(n)

    # 定义时段：早盘 9:30-11:30, 午盘 13:00-14:00, 尾盘 14:00-15:00
    morning_mask = (hours == 9) & (minutes >= 30) | (hours == 10) | (hours == 11) & (minutes <= 30)
    noon_mask = (hours == 13) | (hours == 14) & (minutes == 0)
    afternoon_mask = (hours == 14) & (minutes > 0) | (hours == 15)

    # 主动买卖金额的时段分布（需先计算 side）
    if 'side' in g.columns:
        side = g['side'].astype(str).str.upper()
        bm = side.isin(['B', 'BUY', '1']).values
        sm = side.isin(['S', 'SELL', '-1']).values
    else:
        # 用 price_change 推导
        pchg = np.diff(prices, prepend=prices[0])
        bm = pchg > 0.001
        sm = pchg < -0.001

    buy_amt = tick_amts * bm.astype(float)
    sell_amt = tick_amts * sm.astype(float)
    total_buy = np.sum(buy_amt) + 1e-8
    total_sell = np.sum(sell_amt) + 1e-8

    f['ts_buy_morning_pct'] = np.sum(buy_amt[morning_mask]) / total_buy if total_buy > 0 else 0.0
    f['ts_buy_noon_pct'] = np.sum(buy_amt[noon_mask]) / total_buy if total_buy > 0 else 0.0
    f['ts_buy_afternoon_pct'] = np.sum(buy_amt[afternoon_mask]) / total_buy if total_buy > 0 else 0.0

    # 买入强度的时序趋势（尾盘/早盘 比值）
    f['ts_buy_trend'] = (f['ts_buy_afternoon_pct'] + 1e-8) / (f['ts_buy_morning_pct'] + 1e-8)

    # 早盘拉升 vs 尾盘拉升（价格变化幅度）
    if np.sum(morning_mask) > 0:
        morn_prices = prices[morning_mask]
        f['ts_morning_rally'] = (morn_prices[-1] - morn_prices[0]) / (morn_prices[0] + 1e-8) if len(morn_prices) > 1 else 0.0
    else:
        f['ts_morning_rally'] = 0.0

    if np.sum(afternoon_mask) > 0:
        aft_prices = prices[afternoon_mask]
        f['ts_afternoon_rally'] = (aft_prices[-1] - aft_prices[0]) / (aft_prices[0] + 1e-8) if len(aft_prices) > 1 else 0.0
    else:
        f['ts_afternoon_rally'] = 0.0

    # ---- 订单簿动态特征 ----
    if 'bids' in g.columns and 'asks' in g.columns:
        # 从 JSON 字符串提取第一层 bid/ask 价格序列
        bid1_prices = []
        ask1_prices = []
        for bids_str, asks_str in zip(g['bids'].values, g['asks'].values):
            bp, bv = parse_order_book(bids_str)
            ap, av = parse_order_book(asks_str)
            bid1_prices.append(bp[0] if bp else np.nan)
            ask1_prices.append(ap[0] if ap else np.nan)
        bid1_prices = np.array(bid1_prices)
        ask1_prices = np.array(ask1_prices)
        valid = ~np.isnan(bid1_prices) & ~np.isnan(ask1_prices)
        if np.sum(valid) > 3:
            spreads = ask1_prices[valid] - bid1_prices[valid]
            f['ts_imbalance_volatility'] = np.std(spreads) / (np.mean(spreads) + 1e-8)
        else:
            f['ts_imbalance_volatility'] = 0.0
    else:
        f['ts_imbalance_volatility'] = 0.0

    # ---- 大单时序爆发特征 ----
    if 'tick_big_order_volume' in g.columns:
        big_vols = g['tick_big_order_volume'].values.astype(float)
        # 大单是否集中在某段时间（前20%时间窗口占比）
        top20_len = max(1, int(n * 0.2))
        big_sorted_idx = np.argsort(big_vols)[::-1][:top20_len]
        time_positions = big_sorted_idx / n  # 归一化到 [0,1]
        f['ts_big_order_burst'] = np.std(time_positions)  # 标准差小=集中，大=分散
    else:
        f['ts_big_order_burst'] = 0.0

    # ---- 收益率波动率（简单版） ----
    if len(returns) > 1:
        f['ts_return_volatility'] = np.std(returns)
        # 波动率变化（后半段 vs 前半段）
        mid = len(returns) // 2
        f['ts_vol_shift'] = (np.std(returns[mid:]) + 1e-8) / (np.std(returns[:mid]) + 1e-8) if mid > 0 else 1.0
    else:
        f['ts_return_volatility'] = 0.0
        f['ts_vol_shift'] = 1.0

    # ---- VWAP 趋势偏离 ----
    if 'averageprice' in g.columns and g['averageprice'].notna().any():
        avg_prices = g['averageprice'].values.astype(float)
        mid_idx = n // 2
        if mid_idx > 0 and mid_idx < n:
            vwap_first = np.mean(avg_prices[:mid_idx])
            vwap_last = np.mean(avg_prices[mid_idx:])
            f['ts_vwap_trend'] = (vwap_last - vwap_first) / (vwap_first + 1e-8)
        else:
            f['ts_vwap_trend'] = 0.0
    else:
        f['ts_vwap_trend'] = 0.0
    ...
    # ---- 交易节奏特征 ----
    if 'interval_ms' in g.columns or 'datetime' in g.columns:
        if 'interval_ms' in g.columns:
            intervals = g['interval_ms'].values.astype(float)
        else:
            dt_vals = g['datetime'].values
            intervals = np.diff(dt_vals.astype('datetime64[ms]').astype(float))
            intervals = np.append(intervals, intervals[-1] if len(intervals) > 0 else 0)
        if len(intervals) > 1 and np.mean(intervals) > 0:
            mid = len(intervals) // 2
            f['ts_trade_pace_speedup'] = (np.mean(intervals[mid:]) + 1e-8) / (np.mean(intervals[:mid]) + 1e-8)
        else:
            f['ts_trade_pace_speedup'] = 1.0
    else:
        f['ts_trade_pace_speedup'] = 1.0

    # ---- 撤单加速度 ----
    cancel_cols = ['cancel_volume', 'cancel_count', 'fast_cancel_count']
    has_cancel = all(c in g.columns for c in cancel_cols)
    if has_cancel and g['cancel_count'].sum() > 0:
        cv = g['cancel_volume'].values.astype(float)
        cc = g['cancel_count'].values.astype(float)
        fc = g['fast_cancel_count'].values.astype(float)
        mid = n // 2
        if mid > 0:
            f['ts_cancel_acceleration'] = (np.sum(cc[mid:]) + 1e-8) / (np.sum(cc[:mid]) + 1e-8)
            f['ts_fast_cancel_surge'] = (np.sum(fc[mid:]) + 1e-8) / (np.sum(fc[:mid]) + 1e-8)
        else:
            f['ts_cancel_acceleration'] = 1.0
            f['ts_fast_cancel_surge'] = 1.0
    else:
        f['ts_cancel_acceleration'] = 1.0
        f['ts_fast_cancel_surge'] = 1.0
...
def extract_all_feature_baseline(df_raw):
    ...
    for idx, ((sc, td), g) in enumerate(grouped):
        ...
        # ========== 时序特征提取（提分路径五：时间序列建模）==========
        f = extract_temporal_features(g, f)
    ...
    n_ts = sum(1 for c in df_feat.columns if c.startswith('ts_'))
    print(f"特征提取完成 | {n_feat}维（含{n_ts}维时序特征）| {df_feat.shape[0]}样本")
    return df_feat
...
def task2_capital_recognition(df_feat):
    ...
    if XGBOOST_AVAILABLE and len(df) >= 10 and n_classes >= 2:
        print("\n[*] 训练 XGBoost 分类器...")
        ...
        if not XGBOOST_AVAILABLE:
            reason.append("xgboost 未安装")
        if len(df) < 10:
            reason.append("样本不足")
        if n_classes < 2:
            reason.append(f"伪标签仅包含 {n_classes} 个类别")
        print(f"\n[*] 回退到规则打分（{'，'.join(reason)}）")
...
def save_results_baseline(df_pat, df_res, df_feat, out_dir):
    ...
    last_date = df_pat['transaction_date'].max()
    df_pat = df_pat[df_pat['transaction_date'] == last_date].copy()
    df_res = df_res[df_res['transaction_date'] == last_date].copy()
    print(f"  已过滤：只保留最后一天的 {len(df_pat)} 条记录（日期: {last_date}）")
...
def run_baseline(input_path, output_dir, use_temporal=True, model_type='lstm', multi_day=False, window_days=5):
    print(f"\n{'='*65}")
    ...
    # ========== 时序模型增强（提分路径五增强版：双模式融合）==========
    if use_temporal and TORCH_AVAILABLE:
        print(f"\n{'='*65}")
        print(f"[阶段2.5/2] 时序模型双模式融合")
        print(f"{'='*65}")
        
        # 模式A：单天（日内交易模式）
        print("\n>>> 训练模式A：单天时序（日内行为）...")
        model_a, preds_a = train_temporal_model(
            df_raw, df_res, model_type=model_type, multi_day=False)
        
        # 模式B：跨天滑动窗口（跨天行为演变）
        print("\n>>> 训练模式B：跨天滑动窗口（跨天行为）...")
        model_b, preds_b = train_temporal_model(
            df_raw, df_res, model_type=model_type, multi_day=True, window_days=window_days)
        
        # 三模型融合：静态 + 模式A + 模式B
        if preds_a or preds_b:
            df_res = merge_three_model_predictions(df_res, preds_a, preds_b, 
                alpha_static=0.4, alpha_temporal_a=0.35, alpha_temporal_b=0.25)
            print(f"  已融合静态+单天时序+跨天时序三模型预测")
    elif use_temporal and not TORCH_AVAILABLE:
        print("\n[!] PyTorch 未安装，跳过时序模型（pip install torch）")
...
def main():
    ...
    parser.add_argument('--temporal-model', choices=['lstm', 'transformer'], default='lstm',
                       help='时序模型类型（默认 lstm）')
    parser.add_argument('--temporal-multi-day', action='store_true',
                       help='使用时序跨天滑动窗口模式（一只股票连续多天）')
    parser.add_argument('--temporal-window', type=int, default=5,
                       help='跨天滑动窗口天数（默认5，仅--temporal-multi-day生效）')
    ...
    run_baseline(input_file, args.output,
                ...
                multi_day=args.temporal_multi_day,
